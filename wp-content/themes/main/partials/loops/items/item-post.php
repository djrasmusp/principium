<a href="<?= get_permalink($post->ID) ?>"
   class="relative aspect-square overflow-hidden transition-all group duration-300 focus-visible:z-50 focus-visible:outline-8 focus-visible:outline-offset-4 focus-visible:ring-4 focus-visible:ring-darkgrey focus-visible:outline-darkgrey umami--click--blog-<?= $post->post_name ?>">
    <div class="absolute right-2 bottom-2 left-2 z-20 lg:right-6 lg:bottom-6 lg:left-6">
        ...
        <time datetime="<?= date_i18n('c', $post->post_date) ?>" class="text-xs uppercase text-zinc-100 font-header"><?= date_i18n('d. M Y', $post->post_date) ?></time>
    </div>
    <div class="h-full">
