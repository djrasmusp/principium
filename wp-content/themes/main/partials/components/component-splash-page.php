...
if (class_exists('TRP_Translate_Press')) :
    ...
    if (!is_admin()) : ?>
        <div class="inset-0 bg-splash-page-bg absolute z-9999 flex flex-col items-center justify-center "
             ...
             x-trap.inert="show_splash_page"
             x-cloak
             x-transition:leave="transition ease-in duration-700 delay-200 origin-top"
             x-transition:leave-start="translate-y-0"
             x-transition:leave-end="-translate-y-full"
        >
            <button class="-mt-24 focus:outline-0 h-[480px] w-[480px]" title="<?= get_bloginfo( 'name' ) ?>" @click="show_splash_page = false">
            ...
            <ul class="flex justify-between gap-8 item-center splash-page-language-selector" data-no-translation>
                ...
                <?php foreach ($languages as $lang) : ?>
                    <li x-data="{ url : '<?= $lang['current_page_url'] ?>', redirect : <?= var_export($lang['current_page_url'] !== $actual_link ) ?>  }">
                        <button class="font-header flex gap-4 items-center text-2xl uppercase text-splash-page-text leading-loose tracking-widest transition-all duration-700 outline outline-2 outline-transparent outline-offset-4 focus-visible:outline-darkgrey"
