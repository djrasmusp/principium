            ...
            <section class="<?= $this->get_class_name() ?> h-5/6 mb-8 lg:mb-16" id="<?= $this->get_id() ?>">
                <div class="block relative w-full overflow-hidden <?= ($this->is_preview) ? 'h-5/6' : 'h-5/6 min-h-screen-ios' ?>">
                    <div class="absolute flex lg:flex-row flex-col-reverse justify-between w-full items-center bottom-0 left-0 z-10">
                        <header class="min-h-1/6 w-full lg:max-w-fit bg-darkgrey flex flex-col justify-center" data-aos="slide-right"
                                data-aos-duration="700" data-aos-once="true" data-aos-easing="ease-in-out-sine">
                            <time datetime="<?= date_i18n('c', $this->post->post_date) ?>" class="text-xs px-8 lg:px-16 uppercase text-zinc-100 font-header"><?= date_i18n('d. M Y', $this->post->post_date) ?></time>
                            <h1 class="w-full text-xl lg:text-[2rem]/[1.25] lg:py-[1rem] text-pretty text-left px-8 lg:text-left tracking-[0.4rem] lg:px-16 uppercase block text-white font-header"
